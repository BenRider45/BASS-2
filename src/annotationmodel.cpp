...
QVariant AnnotationModel::data(const QModelIndex &index, int role) const
{
    ...
    switch (role) {
    case StartFrameRole: return frame.startFrame;
    case EndFrameRole:   return frame.endFrame;
    case LabelRole:      return frame.label;
    case SyllableIdRole: return frame.syllableId;
    }
    return {};
}
...
QString AnnotationModel::currentFile() const { return m_currentFile; }

void AnnotationModel::setCurrentFile(const QString &file)
{
...
void AnnotationModel::addFrame(int start, int end, const QString &label)
{
    beginInsertRows(QModelIndex(), m_frames.size(), m_frames.size());
    m_frames.append({start, end, label, -1});
    endInsertRows();
    emit countChanged();
}
